...
const art={range:new Image(),tropical:new Image(),pine:new Image(),jungleTree:new Image()};art.range.src='assets/backgrounds/alpine-range.webp';art.tropical.src='assets/backgrounds/tropical-range.webp';art.pine.src='assets/nature/pine-mature.webp';art.jungleTree.src='assets/nature/rainforest-tree.png';
...
levels.push({name:'THE LAST CLIMB',region:'Cloudbase Station',tag:'FINALE · STRAIGHT UP',
 brief:'Cloudbase Station is 10 440 m above you. There is one way there and it is up.',
 objective:'Climb the shaft and land on Cloudbase Station.',
 length:1920,seed:997,theme:'storm',environment:'storm-mountains',climb:true,combat:true,
 people:[],guns:[],cargo:null,wind:0,par:420,beacon:0});
...
function updateCamera(dt){
 ...
 if(L.climb){
  ...
  zoom=damp(zoom,clamp((Math.min(CLIMB_WIDE,1440)+120)/baseVw,1,3.2),1.5,dt);applyView();
...
function operationTarget(){
 ...
 if(L.summit&&cargo?.status==='attached')return{x:cargo.to,text:Math.abs(heli.x-cargo.to)<300&&heli.y<ground(cargo.to)?'LOWER GENERATOR ONTO SUMMIT PAD':L.region.toUpperCase()+' · '+Math.max(0,Math.round((heli.y-ground(cargo.to))*.45))+' m TO CLIMB'};
...
function selectMissions(){
 ...
 {const v=levels[CLIMB_LEVEL],b=document.createElement('button'),saved=readClimb();
  ...
  b.innerHTML='<small>18 · FINALE</small><b>'+v.name+'</b><span>'+v.region+' · 10 440 m straight up</span><em>'
...
function drawOperation(){
 if(L.summit){for(const x of [...L.fieldPads,L.cargo.to]){if(x<camera-180||x>camera+vw+180)continue;const y=ground(x),summit=x===L.cargo.to;label(summit?L.region.toUpperCase()+' · '+Math.round((ground(390)-ground(x))*.45).toLocaleString('en-US')+' m':'MOUNTAIN CAMP · '+Math.round((610-y)*.45)+' m',x,y-92,'#d4eceb',11);line(x-115,y,x-115,y-75,'#a7bbb8',3);const flutter=Math.sin(visualTime*3+x)*4;poly([[x-115,y-75],[x-78,y-70+flutter],[x-115,y-57]],'#f59442');}}
...
function drawClimbBackdrop(){
 ...
 // Recessed canyon faces move independently of the solid foreground shelves.
 for(let layer=0;layer<2;layer++){
  const speed=layer?.18:.07,period=530,offset=cameraY*speed;
  for(let side=0;side<2;side++)for(let row=Math.floor(offset/period)-1;row<(offset+vh)/period+1;row++){
   const y=row*period-offset,n=hash(row*19+side*73+layer*151);
   const width=vw*(.13+n*.11),edge=side?vw:0,dir=side?-1:1;
   poly([[edge,y-80],[edge+dir*width*.48,y+32],[edge+dir*width,y+180],[edge+dir*width*.72,y+340],[edge+dir*width*.2,y+540],[edge,y+600]],layer?'#56778025':'#6b89951c');
   line(edge+dir*width*.48,y+32,edge+dir*width*.65,y+330,'#c5d7cb15',2);
  }
 }
...
function drawAtmosphere(){const p=palette(),sun=L.theme==='sunset',storm=L.theme==='storm';
 ...
 else if(!storm&&!p.snow){const x=vw*.76-camera*.025,y=sun?210:100;glow(x,y,180,sun?'#ffb95b26':'#fff3bc1c');ctx.save();ctx.globalCompositeOperation='screen';for(let i=0;i<5;i++){const shift=Math.sin(visualTime*.13+i)*22;const g=ctx.createLinearGradient(x,y,x-220,600);g.addColorStop(0,'#fff2b60d');g.addColorStop(1,'#fff2b600');poly([[x+i*12,y],[x+i*12+14,y],[x-220+i*65+shift,610],[x-124+i*65+shift,610]],g);}ctx.restore();}
 ...
 for(const band of [{speed:.35,base:548,scale:.72,tint:p.night?'#1f3d4766':'#3a625d5c',step:31},
  ...
  for(let i=first;i<first+Math.ceil(vw/band.step)+4;i++){
   ...
   const tree=natureSprite(L.theme==='jungle');if(tree){ctx.save();ctx.globalAlpha=band.speed>.35?.34:.2;const tw=L.theme==='jungle'?h*.9:h*.66;ctx.drawImage(tree,x+sway-tw/2,y-h,tw,h);ctx.restore();}
  }
 }
...
function drawJungleTree(t){drawNatureTree(t,true);}
// --- Obstacle art -------------------------------------------------------------------------
...
function drawObstacles(){if(L.lost){const g=ctx.createLinearGradient(0,520,0,790);g.addColorStop(0,'#183039e8');g.addColorStop(1,'#34545188');ctx.fillStyle=g;ctx.fillRect(3420,520,880,270);}if(L.theme==='jungle'){const cave=ctx.createLinearGradient(0,590,0,885);cave.addColorStop(0,'#0c242cef');cave.addColorStop(1,'#224441b0');ctx.fillStyle=cave;ctx.fillRect(1810,590,1180,295);for(let x=1830;x<2990;x+=120){poly([[x,595],[x+50,640],[x+90,800],[x+30,875],[x-15,720]],'#50746724');}}for(const o of obstacles){
  if(o.x+o.w<camera-100||o.x>camera+vw+100||o.y+o.h<cameraY-100||o.y>cameraY+vh+100)continue;
  if(o.deck)continue; // The climb renderer owns the exact deck silhouette.
  if(o.type==='pillar')drawPillar(o);
  else if(o.rockGate){
   drawBoulder(o);
   // Mineral seams and hanging fissures stay inside the solid silhouette.
   ctx.save();ctx.beginPath();ctx.rect(o.x,o.y,o.w,o.h);ctx.clip();
   for(let k=0;k<Math.ceil(o.w/96);k++){
    const x=o.x+k*96+hash(k+o.y)*30;
    line(x,o.y+12,x+20,o.y+o.h*.56,'#102d3c70',2);
    line(x+20,o.y+o.h*.56,x+8,o.y+o.h-4,'#17334580',1);
    line(x+3,o.y+13,x+22,o.y+o.h*.54,'#dccdae40',1);
   }
   ctx.restore();
  }
...
// One art family across foreground and parallax; no legacy polygon trees.
const natureCache=new Map();
function natureSprite(tropical=false){
 const img=tropical?art.jungleTree:art.pine;if(!(img?.naturalWidth||img?.width))return null;
 const p=palette(),key=(tropical?'jungle':'pine')+(p.night?'-night':'-day')+(p.snow?'-snow':'');
 if(natureCache.has(key))return natureCache.get(key);
 const c=document.createElement('canvas');c.width=256;c.height=384;const g=c.getContext('2d');if(tropical)g.drawImage(img,39,34,953,1447,0,0,256,384);else g.drawImage(img,0,0,256,384);
 g.globalCompositeOperation='source-atop';
 if(p.snow){g.fillStyle='#d7e8ee22';g.fillRect(0,0,256,384);for(let i=0;i<260;i++){const x=hash(i*13.1)*256,y=hash(i*9.7)*330;g.fillStyle=i%3?'#e6f0f299':'#abc8d17a';g.beginPath();g.ellipse(x,y,1+hash(i)*3,.7+hash(i+1)*1.3,0,0,TAU);g.fill();}}
 if(p.night){g.fillStyle='#12354b70';g.fillRect(0,0,256,384);}else{g.fillStyle=tropical?'#284c3820':'#2b4b4220';g.fillRect(0,0,256,384);}
 natureCache.set(key,c);return c;
}
function drawNatureTree(t,tropical=false){
 const img=natureSprite(tropical);if(!img)return;
 ctx.save();ctx.translate(t.x,t.y+2);const stretch=.88+hash(t.x*.19)*.23;ctx.scale(t.s*stretch,t.s);
 ctx.transform(1,0,Math.sin(visualTime*.9+t.x*.03)*.008+wind*.0005,1,0,0);
 ellipse(2,0,tropical?31:22,3.4,'#09273336');ctx.scale(hash(t.x*2)>.5?-1:1,1);
 const h=tropical?146:126,w=tropical?97:84;ctx.drawImage(img,-w/2,-h,w,h+2);ctx.restore();
}
function drawTree(t){drawNatureTree(t,L.theme==='jungle');}
...
function render(){ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#06151f';ctx.fillRect(0,0,innerWidth,innerHeight);ctx.translate(ox,oy);ctx.scale(scale,scale);ctx.save();ctx.beginPath();ctx.rect(0,0,vw,vh);ctx.clip();if(!reduceMotion){ctx.translate(Math.sin(visualTime*53)*shake*.25,Math.cos(visualTime*47)*shake*.20)}drawBackdrop();ctx.save();ctx.translate(-camera,-cameraY);drawTerrain();drawGroundDetail();if(heli.z>14)drawPlayer();for(const s of scenery){if(s.x<camera-100||s.x>camera+vw+100)continue;s.type==='tree'?drawTree(s):drawRock(s)}drawBase();drawOutpost();drawSceneProps();drawClimb();drawObstacles();drawOperation();drawLost();drawHazardGuides();drawFlightGuides();for(const e of enemies)if(!e.climbRobot&&e.x>camera-100&&e.x<camera+vw+100)drawEnemy(e);for(const d of debris){
...
//
// The shaft is 1500 units wide and 15 000 tall — about 10 440 m — which is roughly three minutes
...
// station above you.
const CLIMB_FLOOR=610,CLIMB_TOP=-22590,CLIMB_WIDE=1920;
const CLIMB_PADS=[
 {x:960,y:CLIMB_FLOOR,name:'GROUND CREW',label:'LAUNCH'},
 {x:430,y:-4200,name:'RELAY MAST',label:'CHECKPOINT 1'},
 {x:1490,y:-8400,name:'QUARRY REFUGE',label:'CHECKPOINT 2'},
 {x:510,y:-12800,name:'CLOUD ANCHOR',label:'CHECKPOINT 3'},
 {x:1410,y:-16800,name:'STORM SHELTER',label:'CHECKPOINT 4'},
 {x:620,y:-20500,name:'SKY WATCH',label:'CHECKPOINT 5'}
];
...
function climbObstacles(){
 ...
 for(let i=1;i<=35;i++){
  ...
  // Six repeating flight rhythms, with gradual difficulty rather than identical gates.
  const styles=['shelf','arch','gantry','chamber','shelf','gantry'];
  const style=styles[(i-1)%styles.length];
  const gap=style==='chamber'?1350:style==='arch'?920:style==='shelf'?lerp(1030,780,t):lerp(800,620,t),swing=lerp(60,210,t);
  ...
  const height=style==='arch'?170:style==='shelf'?110:46;
  if(open-walls>24)out.push({x:walls,y,w:open-walls,h:height,type:'bridge',climb:true,rockGate:style==='arch'||style==='shelf',climbStyle:style});
  if(CLIMB_WIDE-walls-right>24)out.push({x:right,y:y+(style==='shelf'?85:0),w:CLIMB_WIDE-walls-right,h:height,type:'bridge',climb:true,rockGate:style==='arch'||style==='shelf',climbStyle:style});
 }
 ...
 for(let i=1;i<CLIMB_PADS.length;i++)
  out.push({x:CLIMB_PADS[i].x-130,y:CLIMB_PADS[i].y,w:260,h:22,type:'bridge',climb:true,deck:i});
 out.push({x:CLIMB_WIDE*.5-140,y:CLIMB_TOP,w:280,h:26,type:'bridge',climb:true,deck:'station'});
 return out;
}
...
function startClimb(fresh=false){
 ...
 $('modalBody').innerHTML='<p>Cloudbase Station is 10 440 m above you. There is one way there and it is up.</p>'
  +'<p>Weave the gates. Visit all five shelters on alternating sides. Climb past each shelf, cross the gorge, then descend to land. Shelters save progress, refuel, repair and rearm you. Above the rain the cloud deck is dark — <b>L</b> or the LIGHT button turns on the landing light, and you will need it.</p>'
  +'<p>Robot patrols guard the open chambers. FIRE / Space uses the cannon. ROCKET / R launches a rocket. Aim with the nose. Shelters are safe zones.</p>';
...
function buildDrones(){
 ...
 for(let k=3;k<rows.length;k+=2){
  ...
  climb.drones.push({x:CLIMB_WIDE*.5,y,homeY:y,phase:k*1.37,span:200+((k*61)%100),hp:42,max:42,climbRobot:true,type:'drone',cd:2.8,flash:0,aim:0});
 }
 enemies=climb.drones;
}
function readClimb(){try{const d=JSON.parse(localStorage.getItem('flyinLastClimbV3')||'null');return d&&d.version===1?d:null;}catch{return null;}}
function saveClimb(){if(!climb)return;try{localStorage.setItem('flyinLastClimbV3',JSON.stringify({version:1,cp:climb.cp,best:climb.best,crashes:climb.crashes,total:climb.total,landed:[...climb.landed],done:!!climb.done}));}catch{}}
function restoreClimb(){
 const p=CLIMB_PADS[climb.cp];
 heli.x=p.x;heli.y=p.y-30.8;heli.vx=heli.vy=heli.angle=heli.av=heli.bank=0;
 heli.landed=true;heli.airborne=false;heli.hp=100;heli.fuel=100;
 heli.rockets=8;heli.flares=4;heli.heat=0;heli.overheated=false;
 bullets=[];rockets=[];missiles=[];
...
function updateClimb(dt){
 ...
 for(const d of climb.drones){
  ...
  d.cd-=dt;d.flash=Math.max(0,d.flash-dt);
  const distance=Math.hypot(d.x-heli.x,d.y-heli.y);
  d.aim=Math.atan2(heli.y-d.y,heli.x-d.x);
  const nearPad=CLIMB_PADS.some(p=>Math.abs(heli.y-p.y)<230);
  d.warn=distance<650&&d.cd<.8&&!nearPad;
  if(d.cd<=0&&distance<650&&!nearPad&&!blocked(d.x,d.y,heli.x,heli.y)){
   bullets.push({x:d.x,y:d.y,px:d.x,py:d.y,vx:Math.cos(d.aim)*195,vy:Math.sin(d.aim)*195,life:3.5,enemy:true});
   d.cd=3.2;d.flash=.15;AudioState.sfx('enemyGun',d.x);
  }
 ...
 for(let i=1;i<CLIMB_PADS.length;i++){
  ...
  if(!heli.landed||Math.abs(heli.x-p.x)>130||Math.abs(heli.y+30.8-p.y)>26||Math.abs(heli.vx)>30)continue;
 ...
 if(!climb.arrived&&climb.landed.size===CLIMB_PADS.length&&deck?.deck==='station'&&heli.landed&&Math.abs(heli.vx)<30){
  climb.hold+=dt;
...
function drawClimb(){
 ...
 for(const side of [0,1]){
  ...
  // World-anchored strata: no repeating sawtooth edge or camera-relative texture crawl.
  ctx.save();ctx.beginPath();ctx.rect(x,top,w,bot-top);ctx.clip();
  const shade=ctx.createLinearGradient(x,0,x+w,0);
  shade.addColorStop(0,side?'#788b88':'#182f3b');shade.addColorStop(.55,'#465d63');shade.addColorStop(1,side?'#1b303d':'#91a49a');
  ctx.fillStyle=shade;ctx.fillRect(x,top,w,bot-top);
  for(let y=Math.floor(top/117)*117;y<bot;y+=117){
   const n=hash(y*.021+side*5.3),rise=n*37;
   poly([[x,y],[x+w,y+rise],[x+w,y+61],[x+26,y+94],[x,y+70]],n>.5?'#a5b2a529':'#071c334f');
   line(x,y+7,x+w,y+rise+7,'#bec7b838',2);
   line(x+20,y+16,x+31,y+57,'#0b223b66',2);
   line(x+31,y+57,x+18,y+86,'#0b223b66',1);
   for(let k=0;k<8;k++)ellipse(x+hash(y+k)*70,y+hash(y+k*17)*100,1.3,.7,'#c1c8b62e');
  }
  ctx.restore();line(inner,top,inner,bot,'#bbc7b649',1.5);
 }
 // Ledges you can stand on, and the mast that carries each one.
 const sectors=['LOWER QUARRY','RELAY GORGE','PATROL CHAMBERS','CLOUD CROSSING','UPPER RIDGE','FINAL APPROACH'];
 for(let i=0;i<sectors.length;i++){
  const y=CLIMB_FLOOR-700-i*3800;if(y<top||y>bot)continue;
  const x=106;
  line(x,y-26,x,y+25,'#dfb777',3);
  label(String(i+1).padStart(2,'0'),x+26,y,'#f2d398',16);
  label(sectors[i],x+124,y+23,'#c4d8d5',10);
 }
 for(let i=1;i<CLIMB_PADS.length;i++){
  const p=CLIMB_PADS[i];
  if(p.y<top-300||p.y>bot+300)continue;
  const x=p.x;
  ctx.fillStyle='#2c3f4a';ctx.fillRect(x-130,p.y,260,22);
  ctx.fillStyle='#3d5763';ctx.fillRect(x-130,p.y,260,5);
  ...
  drawMissionProp(['medical_case','generator','cargo_crate','fuel_can','beacon'][i-1],x+104,p.y-2,.55);
  drawPerson({x:x-106,y:p.y-10,status:'sheltered',phase:i*1.7,character:'rescue_worker',visualState:on?'idle':'wave'});
 ...
 // Drones.
 for(const d of climb.drones){
  if(d.hp<=0||d.y<top-200||d.y>bot+200)continue;
  drawRobot(d.x,d.y,.85,true,d.hp/d.max,1,d.aim,d.flash>0);
  if(d.warn){ctx.save();ctx.setLineDash([5,9]);line(d.x,d.y,heli.x,heli.y,'#ffb66a55',1);ctx.restore();}
...
function drawStation(top,bot){
 ...
 // The deck.
 ctx.fillStyle='#2e4551';ctx.fillRect(x-140,y,280,26);
 ctx.fillStyle='#43616e';ctx.fillRect(x-140,y,280,6);
 for(let k=-2;k<=2;k++){line(x+k*54,y+26,x+k*44,y+100,'#243743',5);}
 for(const edge of [-140,140]){line(x+edge,y,x+edge,y+26,'#efc681',3);glow(x+edge,y,20,'#ffd68a30');}
 for(let k=-3;k<=3;k++)line(x+k*36,y+9,x+k*36+14,y+21,'#d4a75c',3);
 // Landing circle.
 ...
 // Tower and mast.
 const facade=ctx.createLinearGradient(x+72,0,x+132,0);facade.addColorStop(0,'#91a8ac');facade.addColorStop(.22,'#537680');facade.addColorStop(1,'#233f4c');
 ctx.fillStyle=facade;ctx.fillRect(x+72,y-104,60,104);
 poly([[x+68,y-104],[x+77,y-113],[x+128,y-113],[x+138,y-104]],'#c0c6b6');
 line(x+72,y-101,x+132,y-101,'#f0c581',2);
 for(let r=0;r<3;r++)for(let c=0;c<2;c++){
  const wx=x+78+c*27,wy=y-92+r*28;
  ctx.fillStyle='#172d39';ctx.fillRect(wx,wy,22,18);
  ctx.fillStyle=r+c===0?'#ead39b':'#82b6c5';ctx.fillRect(wx+2,wy+2,18,13);
  line(wx+4,wy+4,wx+16,wy+4,'#dce9dc99',1);
 }
 for(let r=1;r<4;r++)line(x+73,y-r*27,x+131,y-r*27,'#c2d2ce33',1);
 line(x-124,y,x-124,y-190,'#5f7b83',5);
 for(let k=1;k<5;k++)line(x-132,y-k*38,x-116,y-k*38+14,'#5f7b83',2);
 ellipse(x-124,y-192,6,6,Math.sin(visualTime*4)>0?'#ff6a4a':'#7d3a2c');
 // The crew. Fourteen of them, waving.
 for(let i=0;i<14;i++){
  const px=x-112+i*17;
  drawPerson({x:px,y:y-10,status:'sheltered',phase:i*1.7,character:i%3?'civilian':'rescue_worker',visualState:'wave'});
 }
 label('CLOUDBASE STATION',x,y-215,'#ffe9bd',15);
 label('10 440 m',x,y-196,'#a9d6cf',11);
...
function updateHUD(){
 ...
 $('lightBtn').hidden=!L.climb||!coarse||mode!=='playing';
 $('winchBtn').hidden=!!L.climb;
 if(L.climb){
  ...
  const next=CLIMB_PADS.find((p,i)=>i>0&&!climb?.landed.has(i));
  const destination=next?next.label+' '+(next.x<heli.x?'←':'→')+' '+(next.y<heli.y?'↑':'↓')+' '+Math.round(Math.hypot(next.x-heli.x,next.y-heli.y)*.45)+' m':'CLOUDBASE STATION · ↑ '+climbMetres(left)+' m TO GO';
  put('compactGoal',destination+' · '+weather,true);
