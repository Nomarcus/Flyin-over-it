...
function selectMissions(){
 mode='select';$('menu').hidden=true;showModal('NORDIC AIR RESCUE','Seven calls. Your helicopter.','<div class="missionlist operations" id="missionList"></div><p>The expedition plus all seven calls are open. Fly at your own pace; improve your precision and star rating on each return.</p>',[{text:'MAIN MENU',run:toMenu}]);
 const list=$('missionList');
 // The expedition is not one of the seven calls, but it is the long course the game is named
 ...
 operations.forEach((l,i)=>{const b=document.createElement('button'),r=save.results[OP_START+i];b.className='operationCard';b.innerHTML='<small>0'+(i+1)+' · '+l.tag+'</small><b>'+l.name+'</b><span>'+l.region+' · '+(l.length*.45/1000).toFixed(1)+' km across</span><em>'+(r?'★'.repeat(r.stars)+'☆'.repeat(3-r.stars)+' · '+fmt(r.score)+' points':'MISSIONS →')+'</em>';b.onclick=()=>loadLevel(OP_START+i);list.append(b);});
}
