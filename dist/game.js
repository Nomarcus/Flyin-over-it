...
function loadLevel(i,play=true){if(play&&levels[i]?.ops){save.lastOperation=i;persist();}lost.active=false;$('lostStatus').hidden=true;document.body?.classList.remove('lostMode');$('skipSchool').hidden=true;$('retrySchool').hidden=true;school={active:false,stage:0,hold:0};precision={hold:0,approach:0,fast:false,targets:new Set(),landings:new Set()};level=i;L={...levels[i],guns:[],boss:false,clear:false};if(!L.lost&&!L.ops){L.brief='Fly carefully, rescue the crew and return to base.';L.objective=L.cargo?'Deliver the cargo and bring everyone home.':'Rescue everyone and return to base.';}makeWorld();
...
function updateOperation(dt){
 ...
 for(const l of ops.lakes)if(heli.x>l.x&&heli.x<l.x+l.w&&heli.y+15>l.y){hitHeli(2);if(!ops.heatNotice){radio('Only the bucket belongs in the lake. Keep the helicopter above the water.',4);ops.heatNotice=4;}}
...
function operationBriefing(){showModal('MISSION '+(L.opsIndex+1)+' / 6 · '+L.tag,L.name,'<p>'+L.brief+'</p><p class="missionhint">'+L.objective+'</p><p>'+((L.fires||[]).length?'E / BUCKET: lower and raise. The bucket fills automatically in a lake. Space / DROP WATER: start or stop the release.':'E / WINCH: hold to lower, release to raise. On touch screens: tap to toggle.')+'</p><p>No hard time limit. Every mission is available to select and replay.</p>',[{text:'START MISSION',primary:true,run:begin},{text:'MISSIONS',run:selectMissions}]);}
function selectMissions(){
 mode='select';$('menu').hidden=true;showModal('NORDIC AIR RESCUE','Six calls. Your helicopter.','<div class="missionlist operations" id="missionList"></div><p>All six are open. Fly at your own pace; improve your precision and star rating on each return.</p>',[{text:'MAIN MENU',run:toMenu}]);
 const list=$('missionList');operations.forEach((l,i)=>{const b=document.createElement('button'),r=save.results[OP_START+i];b.className='operationCard';b.innerHTML='<small>0'+(i+1)+' · '+l.tag+'</small><b>'+l.name+'</b><span>'+l.region+' · '+(l.length*.45/1000).toFixed(1)+' km across</span><em>'+(r?'★'.repeat(r.stars)+'☆'.repeat(3-r.stars)+' · '+fmt(r.score)+' points':'MISSIONS →')+'</em>';b.onclick=()=>loadLevel(OP_START+i);list.append(b);});
}
...
function killBoss(){explode(boss.x,boss.y,2.1);score+=2200;popup(boss.x,boss.y-55,'HEAVY GUNSHIP +2 200');radio('Gunship disabled! Pick up the others and come home.',6);for(let i=0;i<6;i++)debris.push({x:boss.x+rand(-50,50),y:boss.y,vx:rand(-100,100),vy:rand(-80,0),s:rand(.3,.7),type:'falling'});}
