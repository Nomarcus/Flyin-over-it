...
const art={day:new Image(),night:new Image(),jungle:new Image()};art.day.src='alpine-expedition.webp';art.night.src='night-expedition.webp';art.jungle.src='jungle-expedition.webp';
...
// Six self-contained operations. Earlier prototypes remain archived for regression coverage only.
const OP_START=levels.length;
const operations=[
 {name:'FÖRSTA LARMET',region:'Björksjön',tag:'RÄDDNING · LUGN DAG',brief:'Två vandrare behöver hjälp på varsin sida av klippåsen. Välj hög väg över åsen eller flyg försiktigt runt den. Vinscha upp båda och landa hemma.',objective:'Rädda två vandrare och flyg hem.',length:2400,seed:417,theme:'day',people:[1120,1980],wind:0,par:230,obstacles:[{x:1520,w:95,h:160,type:'pillar'}]},
 {name:'SKOGSBRANDEN',region:'Kopparsjön',tag:'VATTEN · PRECISION',brief:'Två brandhärdar hotar skogen. Sänk skopan i den turkosa sjön tills den fylls. Flyg över elden och tryck SLÄPP VATTEN. Vattnet behåller din fart: bromsa eller sikta lite före målet. Återvänd när båda bränderna är släckta.',objective:'Släck två bränder och återvänd.',length:3400,seed:462,theme:'sunset',people:[],wind:4,par:330,lakes:[{x:860,w:480,y:620}],fires:[{x:2110,water:60},{x:2840,water:70}],obstacles:[{x:1590,w:90,h:145,type:'pillar'}]},
 {name:'FYREN UTAN STRÖM',region:'Nattudden',tag:'TUNG LAST · NATTRÄDDNING',brief:'Fyren har slocknat. Hämta generatorn vid depån och sänk den på fyrens platta. När strömmen är tillbaka kan två fyrvaktare lämna skyddsrummet. Hämta dem och flyg hem. Mellanplattan ger bränsle och reparation.',objective:'Ge fyren ström, rädda två och återvänd.',length:4400,seed:483,theme:'night',people:[3460,3850],cargo:{x:1000,to:3110},gate:'cargo',wind:6,par:440,fieldPads:[2240],obstacles:[{x:1570,w:80,h:195,type:'pillar'},{x:2640,w:230,h:70,type:'bridge',gap:260}]},
 {name:'SMARAGDSPASSET',region:'Orkidéexpeditionen',tag:'RAVIN · GROTTA · VINSCH',brief:'Tre forskare är utspridda längs ravinen. En väntar under klippvalvet. De turkosa lamporna visar öppningen; över berget finns en friare väg för resten av flygningen. Mellanlanda vid lägret vid behov. Rädda alla tre och återvänd.',objective:'Rädda tre forskare, en under klippvalvet.',length:5400,seed:503,theme:'jungle',people:[1470,2420,4630],wind:7,par:510,fieldPads:[3730],obstacles:[{x:1810,y:425,w:1180,h:165,type:'roof'},{x:3260,w:90,h:170,type:'pillar'},{x:4230,w:85,h:150,type:'pillar'}]},
 {name:'GLÖDLINJEN',region:'Evakueringszonen',tag:'BRANDSLÄCKNING → EVAKUERING',brief:'Tre bränder spärrar vägen ut för byborna. Hämta vatten i någon av sjöarna och släck alla härdar. Därefter byts skopan automatiskt mot räddningskroken. Hämta de tre byborna och flyg hem. Mellanplattan ger en paus mellan insatserna.',objective:'Släck tre bränder, evakuera tre personer.',length:6600,seed:547,theme:'sunset',people:[3820,4910,5840],gate:'fire',wind:11,par:660,lakes:[{x:950,w:500,y:625},{x:4270,w:420,y:625}],fires:[{x:3720,water:70},{x:5040,water:80},{x:5830,water:75}],fieldPads:[2810],obstacles:[{x:2080,w:110,h:230,type:'pillar'},{x:3220,w:240,h:80,type:'bridge',gap:255},{x:5370,w:85,h:180,type:'pillar'}]},
 {name:'SISTA LJUSET',region:'Frostkedjan',tag:'VINTER · TRANSPORT → RÄDDNING',brief:'Vinterstationen har förlorat värmen. Flyg generatorn genom bergspasset till stationen. Fyra personer kan därefter lämna sina nödbivacker. Två mellanplattor ger bränsle och reparation. Välj de öppna höga rutterna när lastens pendling blir svår att hantera, och för alla hem.',objective:'Leverera värmen och rädda fyra personer.',length:8000,seed:591,theme:'snow',people:[4050,5240,6340,7250],cargo:{x:1070,to:3620},gate:'cargo',wind:13,par:800,fieldPads:[2560,5870],obstacles:[{x:1760,w:100,h:220,type:'pillar'},{x:3040,w:240,h:85,type:'bridge',gap:275},{x:4640,w:110,h:240,type:'pillar'},{x:6710,w:320,h:110,type:'roof',gap:250}]}
];
operations.forEach((l,i)=>levels.push({...l,ops:true,opsIndex:i,guns:[]}));
...
function makeWorld(){const rng=seeded(L.seed);obstacles=[];if(L.lost)obstacles=[
 ...
 }scenery=[];for(let x=690;x<L.length-120;x+=40+rng()*95){scenery.push({x,y:ground(x),s:.7+rng()*1.1,type:rng()>.26?'tree':'rock',variant:rng(),depth:rng()})}if(L.theme==='jungle')for(const t of scenery)if(t.x>1810&&t.x<2990){t.type='rock';t.s*=.65;}if(L.ops)scenery=scenery.filter(t=>!(L.lakes||[]).some(l=>t.x>l.x-25&&t.x<l.x+l.w+25)&&!(L.fieldPads||[]).some(x=>Math.abs(x-t.x)<100)&&!(L.fires||[]).some(f=>Math.abs(f.x-t.x)<75));clouds=Array.from({length:12},()=>({x:rng()*5000,y:45+rng()*240,w:130+rng()*190,a:.04+rng()*.07}));}
...
function loadLevel(i,play=true){lost.active=false;$('lostStatus').hidden=true;document.body?.classList.remove('lostMode');$('skipSchool').hidden=true;$('retrySchool').hidden=true;school={active:false,stage:0,hold:0};precision={hold:0,approach:0,fast:false,targets:new Set(),landings:new Set()};level=i;L={...levels[i],guns:[],boss:false,clear:false};if(!L.lost&&!L.ops){L.brief='Flyg varsamt, hjälp besättningen och återvänd till basen.';L.objective=L.cargo?'Leverera lasten och rädda alla till basen.':'Rädda alla och återvänd till basen.';}makeWorld();
...
function updateBucketWinch(dt){
 ...
 const lake=ops.lakes.find(l=>h.hookX>l.x+12&&h.hookX<l.x+l.w-12&&h.hookY+17>=l.y&&h.hookY<l.y+45);
 if(lake&&!ops.dropping&&Math.abs(h.vx)<65&&Math.abs(h.vy)<65){ops.water=Math.min(100,ops.water+dt*50);if(ops.water>=99&&!ops.fillNotice){ops.fillNotice=true;radio('Full skopa. Hissa upp och flyg till branden. Släpp vattnet med mellanslag eller vattenknappen.',5);AudioState.sfx('attach');}}
...
function waterImpact(x,y,amount){
 for(const f of ops.fires){if(f.out||Math.abs(x-f.x)>74||Math.abs(y-f.y)>65)continue;const used=Math.min(amount,f.left);f.left=Math.max(0,f.left-used);ops.hits+=used;
  if(f.left<=.01){f.out=true;score+=600;AudioState.sfx('rescue');radio('Brand '+(f.id+1)+' släckt. '+ops.fires.filter(k=>!k.out).length+' kvar.',3);}
 }
}
function updateOperation(dt){
 ...
 for(const f of ops.fires){if(!f.out&&Math.abs(heli.x-f.x)<75&&heli.y>f.y-65){hitHeli(1.2);if(!ops.heatNotice){radio('Hett nära marken. Släpp vattnet från högre höjd.',3);ops.heatNotice=4;}}}
 for(const l of ops.lakes)if(heli.x>l.x&&heli.x<l.x+l.w&&heli.y+15>l.y){hitHeli(2);if(!ops.heatNotice){radio('Bara skopan ska ner i sjön. Håll helikoptern över vattnet.',4);ops.heatNotice=4;}}
 const fireDone=ops.fires.every(f=>f.out);
 if(ops.bucket&&fireDone){ops.bucket=false;ops.water=0;ops.dropping=false;heli.ropeTarget=null;keys.KeyE=false;radio(people.length?'Bränderna är släckta. Räddningskroken är klar – hämta människorna.':'Alla bränder släckta. Återvänd och landa hemma.',6);}
 if(!ops.unlocked&&((L.gate==='fire'&&fireDone)||(L.gate==='cargo'&&cargo?.status==='delivered'))){ops.unlocked=true;people.forEach(p=>{if(p.status==='sheltered')p.status='waiting';});if(L.gate==='cargo')radio('Ström och värme tillbaka. Besättningen lämnar skyddet – hämta dem.',5);}
 const field=(L.fieldPads||[]).find(x=>Math.abs(heli.x-x)<65);
 if(field!==undefined&&heli.landed&&Math.abs(heli.vx)<15){ops.fieldTime+=dt;heli.fuel=Math.min(100,heli.fuel+dt*18);heli.hp=Math.min(100,heli.hp+dt*10);repairDents(dt*.12);if(ops.fieldTime>1&&ops.fieldTime-dt<=1)radio('Mellanlandning. Vi fyller på och reparerar. Målet finns kvar på instrumentraden.',4);}else ops.fieldTime=0;
}
function operationTarget(){
 const nearest=list=>list.slice().sort((a,b)=>Math.abs(a.x-heli.x)-Math.abs(b.x-heli.x))[0];
 if(ops.bucket){if(ops.water<12&&!ops.dropping){const l=nearest(ops.lakes.map(l=>({x:l.x+l.w/2})));return{x:l.x,text:'FYLL SKOPAN I SJÖN'};}const f=nearest(ops.fires.filter(f=>!f.out));return{x:f?.x||390,text:'SLÄCK BRAND · '+ops.fires.filter(f=>f.out).length+'/'+ops.fires.length};}
 if(cargo&&cargo.status!=='delivered')return{x:cargo.status==='attached'?cargo.to:cargo.x,text:cargo.status==='attached'?'SÄNK GENERATORN VID PLATTAN':'HÄMTA GENERATORN'};
 if(heli.ropeTarget?.kind==='person')return{x:heli.x,text:'HISSA UPP · SLÄPP VINSCHEN'};
 const p=nearest(people.filter(p=>p.status==='waiting'));return p?{x:p.x,text:'RÄDDA · '+(heli.carrying+heli.delivered)+'/'+people.length}:{x:390,text:'HEM TILL BASEN'};
}
function operationHint(){
 if(ops.bucket){if(ops.dropping)return 'VATTNET BEHÅLLER DIN FART · TRYCK IGEN FÖR ATT STOPPA';return ops.water<12?'SÄNK SKOPAN I SJÖN · DEN FYLLS AUTOMATISKT':'HISSA UPP SKOPAN · MELLANSLAG / SLÄPP VATTEN ÖVER ELDEN';}
 if(cargo?.status==='attached')return 'TUNG LAST · BROMSA MJUKT OCH LÅT PENDLINGEN LÄGGA SIG';
 if(time<13)return L.opsIndex===0?'LUGN INFÖR RÄDDNING · SÄNK VINSCHEN MED E ELLER KNAPPEN':L.tag;
 return ops.fieldTime>0?'SERVICE PÅGÅR · LANDA KVAR EN STUND':'';
}
function updateOperationHUD(){
 ...
 if(ops?.bucket){put('waterStatus','<small>VATTEN</small><b>'+Math.round(ops.water)+'<em>%</em></b>');put('dropBtn',ops.dropping?'STOPPA VATTEN':'SLÄPP VATTEN',true);$('dropBtn').disabled=ops.water<=0;}
 put('winchBtn',ops?.bucket?'SKOPA ↓ / ↑':'VINSCH',true);
}
function operationBriefing(){showModal('UPPDRAG '+(L.opsIndex+1)+' / 6 · '+L.tag,L.name,'<p>'+L.brief+'</p><p class="missionhint">'+L.objective+'</p><p>'+((L.fires||[]).length?'E / SKOPA: sänk och hissa. Skopan fylls automatiskt i sjön. Mellanslag / SLÄPP VATTEN: starta eller stoppa släppet.':'E / VINSCH: sänk, släpp för att hissa. På mobilen: tryck av/på.')+'</p><p>Ingen hård tidsgräns. Alla uppdrag går att välja och spela om.</p>',[{text:'STARTA UPPDRAG',primary:true,run:begin},{text:'VÄLJ UPPDRAG',run:selectMissions}]);}
function selectMissions(){
 mode='select';$('menu').hidden=true;showModal('NORDIC AIR RESCUE','Sex larm. Din helikopter.','<div class="missionlist operations" id="missionList"></div><p>Alla sex är öppna. Flyg i din takt; förbättra din precision och ditt stjärnbetyg när du spelar om.</p>',[{text:'HUVUDMENY',run:toMenu}]);
 const list=$('missionList');operations.forEach((l,i)=>{const b=document.createElement('button'),r=save.results[OP_START+i];b.className='operationCard';b.innerHTML='<small>0'+(i+1)+' · '+l.tag+'</small><b>'+l.name+'</b><span>'+l.region+' · '+(l.length*.45/1000).toFixed(1)+' km utsträckning</span><em>'+(r?'★'.repeat(r.stars)+'☆'.repeat(3-r.stars)+' · '+fmt(r.score)+' poäng':'VÄLJ UPPDRAG →')+'</em>';b.onclick=()=>loadLevel(OP_START+i);list.append(b);});
}
function drawOperation(){
 if(!ops)return;
 for(const l of ops.lakes){if(l.x+l.w<camera||l.x>camera+vw)continue;
  const g=ctx.createLinearGradient(0,l.y,0,l.y+44);g.addColorStop(0,'#6cd5ce');g.addColorStop(1,'#164d66');poly([[l.x,l.y],[l.x+l.w,l.y],[l.x+l.w,ground(l.x+l.w)],[l.x,ground(l.x)]],g);line(l.x,l.y,l.x+l.w,l.y,'#d6fff0',2);
  for(const x of [l.x+24,l.x+l.w-24]){line(x,l.y,x,l.y-19,'#d8f1da',2);ellipse(x,l.y,6,3,'#e49b4a');poly([[x,l.y-19],[x+12,l.y-14],[x,l.y-10]],'#d9f9d7');}
  for(let x=l.x+12;x<l.x+l.w-8;x+=24){const offset=Math.sin(visualTime*1.6+x*.06)*5;line(x+offset,l.y+7+Math.sin(x)*4,x+14+offset,l.y+7+Math.sin(x)*4,'#c4ffe563',1);}
  if(ops.bucket&&Math.abs(heli.hookX-(l.x+l.w/2))<l.w/2){ctx.save();ctx.strokeStyle='#d6fff199';ctx.lineWidth=1;ctx.beginPath();ctx.ellipse(heli.hookX,l.y+2,24+Math.sin(visualTime*5)*4,4,0,0,TAU);ctx.stroke();ctx.restore();}
 }
 ...
 for(const f of ops.fires){if(f.x<camera-180||f.x>camera+vw+180)continue;const strength=f.left/f.max;
  if(L.gate==='fire')building(f.x-37,f.y,74,40,'#876f52');
  ellipse(f.x,f.y+2,76,10,'#152833aa');
  for(let j=0;j<5;j++){const x=f.x-48+j*23;line(x-6,f.y-2,x+9,f.y-18,'#493d33',5);line(x,f.y-4,x+13,f.y-8,'#242b2e',3);}
  if(f.out){ellipse(f.x,f.y,60,5,'#69c8bc38');continue;}
  glow(f.x,f.y-25,115,'#ff963537');
  for(let j=0;j<9;j++){const x=f.x-62+j*15,wave=Math.sin(visualTime*7+j*2.4),height=(32+hash(f.x+j)*40)*(.45+.55*strength);ctx.beginPath();ctx.moveTo(x-10,f.y);ctx.quadraticCurveTo(x-20,f.y-height*.5,x+wave*9,f.y-height);ctx.quadraticCurveTo(x+22,f.y-height*.35,x+10,f.y);ctx.closePath();ctx.fillStyle=j%2?'#f07b27dc':'#ffb843e8';ctx.fill();poly([[x-5,f.y],[x+wave*4,f.y-height*.6],[x+6,f.y]],'#ffe3a0b0');}
  for(let j=0;j<4;j++){const t=(visualTime*.25+j*.25)%1;glow(f.x+Math.sin(j*2)*22+wind*t,f.y-40-t*110,18+t*28,'#35444b35');}
 }
...
function drawTerrain(){
 ...
 const floor=1400,pts=[[a*40,floor]];for(let i=a;i<=b;i++)pts.push([i*40,terrain[i]]);pts.push([b*40,floor]);
 ctx.save();ctx.beginPath();pts.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.closePath();ctx.clip();
 const g=ctx.createLinearGradient(0,500,0,1300);g.addColorStop(0,p.snow?'#7d9296':'#8e8066');g.addColorStop(.38,p.night?'#243543':'#4b5553');g.addColorStop(1,'#142a35');ctx.fillStyle=g;ctx.fillRect(a*40,0,(b-a)*40,1400);
...
function drawTree(t){
 ...
 ellipse(7,2,30,5,'#09273335');poly([[-3,0],[4,0],[2,-89],[-1,-89]],'#665543');line(1,-3,1,-72,'#c4ab7255',1.5);
 for(let j=0;j<6;j++){
  ...
  poly(edge,p.night?'#183d43':j%2?'#214f48':'#2c6152');
...
function render(){ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#06151f';ctx.fillRect(0,0,innerWidth,innerHeight);ctx.translate(ox,oy);ctx.scale(scale,scale);ctx.save();ctx.beginPath();ctx.rect(0,0,vw,vh);ctx.clip();if(!reduceMotion){ctx.translate(Math.sin(visualTime*53)*shake*.25,Math.cos(visualTime*47)*shake*.20)}drawBackdrop();ctx.save();ctx.translate(-camera,-cameraY);drawTerrain();drawGroundDetail();if(heli.z>14)drawPlayer();for(const s of scenery){if(s.x<camera-100||s.x>camera+vw+100)continue;s.type==='tree'?drawTree(s):drawRock(s)}drawBase();drawOutpost();drawObstacles();drawOperation();drawLost();drawHazardGuides();drawFlightGuides();for(const e of enemies)if(e.x>camera-100&&e.x<camera+vw+100)drawEnemy(e);for(const d of debris){
...
function finishMission(){if(mode!=='playing')return;mode='debrief';const timeBonus=Math.max(0,Math.round((L.par-time)*7)),conditionBonus=Math.round(heli.hp*6),total=score+timeBonus+conditionBonus;score=total;const stars=1+(time<=L.par?1:0)+(crashHits===0&&heli.hp>=65?1:0);const previous=save.results[level]||{};save.results[level]={score:Math.max(total,previous.score||0),stars:Math.max(stars,previous.stars||0),time:Math.min(time,previous.time||Infinity)};save.unlocked=Math.max(save.unlocked,Math.min(level+1,levels.length-1));save.best=Math.max(save.best,total);persist();updateHUD();const final=L.ops?L.opsIndex===operations.length-1:level===7;const starline='★'.repeat(stars)+'☆'.repeat(3-stars);showModal(final?'KAMPANJ SLUTFÖRD':'BESÄTTNINGEN ÄR HEMMA',final?'Ingen lämnades kvar.':'Snyggt flugit.',`<div class="medals">${starline}</div><div class="results"><div><b>${fmt(total)}</b><span>POÄNG</span></div><div><b>${Math.floor(time/60)}:${String(Math.floor(time%60)).padStart(2,'0')}</b><span>FLYGTID</span></div><div><b>${heli.delivered}</b><span>RÄDDADE</span></div></div><p>Tidsbonus +${fmt(timeBonus)} · Skrovbonus +${fmt(conditionBonus)}<br>${perfectPickups} precisionsräddningar · ${crashHits} hårda landningar</p>${ops?.fires.length?'<p>'+ops.fires.filter(f=>f.out).length+' bränder släckta · '+Math.round(ops.hits/Math.max(1,ops.used)*100)+' % av vattnet träffade elden</p>':''}<p>★ Uppdraget klart · ★ Under ${Math.round(L.par/60*10)/10} min<br>★ Inga hårda landningar och minst 65 % skrov</p>`,[{text:final?'VÄLJ UPPDRAG':'NÄSTA UPPDRAG',primary:true,run:final?selectMissions:()=>levels[level+1]?.lost?startLost():loadLevel(level+1)},{text:'FLYG IGEN',run:()=>loadLevel(level)},{text:'HUVUDMENY',run:toMenu}]);}
let wreck=null;
...
$('menuSettings').onclick=settings;$('lostBtn').onclick=selectMissions;$('freshLostBtn').onclick=selectMissions;$('startBtn').onclick=selectMissions;$('schoolBtn').onclick=selectTraining;$('skipSchool').onclick=selectTraining;$('retrySchool').onclick=()=>{const kind=school.kind||'basic';startDrill(kind)};$('selectBtn').onclick=selectMissions;$('jungleBtn').onclick=()=>{AudioState.init();loadLevel(OP_START+3)};$('pauseBtn').onclick=pause;$('settingsBtn').onclick=settings;$('soundBtn').onclick=()=>{save.muted=!save.muted;persist();AudioState.init();AudioState.sync();updateHUD()};$('fullBtn').onclick=async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else if(document.documentElement.requestFullscreen)await document.documentElement.requestFullscreen();}catch{}};if(!document.documentElement.requestFullscreen)$('fullBtn').hidden=true;
