...
// Foreground trees and distant trees use the same detailed art, even the formerly legacy 10%.
for(const [name,mission,x] of [['jungle',11,1500],['snow',13,2500],['summit',23,13200]]){
 start(mission);const st=a.get();st.heli.x=x;st.heli.y=a.ground(x)-100;st.heli.landed=false;st.heli.spool=.8;
 const sprite=a.natureSprite(name==='jungle');assert(sprite,'Biome sprite ready');
 a.set({camera:x-st.vw*.45,cameraY:a.ground(x)-st.vh*.65});a.render();fs.writeFileSync('/tmp/finale-'+name+'.png',c.toBuffer('image/png'));
 cx.resetTransform();cx.clearRect(0,0,c.width,c.height);a.drawTree({x:200,y:260,s:1});
 const data=cx.getImageData(140,100,120,150).data;let painted=0,colors=new Set();for(let i=0;i<data.length;i+=4)if(data[i+3]>30){painted++;colors.add((data[i]<<16)|(data[i+1]<<8)|data[i+2]);}
 assert(painted>500&&colors.size>150,'Textured tree, not a polygon silhouette: '+name);
}
console.log('PASS: textured tropical/snow vegetation and high-altitude scene render');
